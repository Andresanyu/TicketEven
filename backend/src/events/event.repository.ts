...
async function fetchEventWithEntradas(client: PoolClient, eventId: number): Promise<EventRow> {
...
export class EventRepository implements IEventRepository {
  ...
  async findAll(): Promise<EventRow[]> {
    const result = await this.pool.query(`${EVENT_SELECT_QUERY} ORDER BY e.id DESC`);
    return result.rows;
  }
  ...
  async patchActivo(id: number, activo: boolean): Promise<EventRow | null> {
    ...
    const eventResult = await this.pool.query(`${EVENT_SELECT_QUERY} WHERE e.id = $1`, [id]);
    return eventResult.rows[0] ?? null;
  }

  async delete(id: number): Promise<boolean> {
    const result = await this.pool.query('DELETE FROM eventos WHERE id = $1', [id]);
    return (result.rowCount ?? 0) > 0;
  }

  async toggleSaved(userId: number, eventId: number): Promise<{ isSaved: boolean }> {
    const eventCheck = await this.pool.query('SELECT id FROM eventos WHERE id = $1', [eventId]);
