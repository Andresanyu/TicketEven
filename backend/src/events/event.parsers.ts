...
export function parseEntradas(value: unknown, required: boolean): ParseEntradasResult {
  ...
  for (const item of value) {
    ...
    const precio = Number((item as any)?.precio); // 👈

    if (!Number.isInteger(tipoEntradaId) || tipoEntradaId <= 0) {
      return {
        entradas: null,
        error: 'Cada entrada debe incluir un tipo_entrada_id entero válido',
      };
    }
    if (!Number.isInteger(aforo) || aforo < 0) {
      return {
        entradas: null,
        error: 'Cada entrada debe incluir un aforo entero mayor o igual a 0',
      };
    }
    if (isNaN(precio) || precio < 0) {
      // 👈
      return { entradas: null, error: 'Cada entrada debe incluir un precio mayor o igual a 0' };
    }
    if (ids.has(tipoEntradaId)) {
      return {
        entradas: null,
        error: 'No se permiten tipos de entrada repetidos en el mismo evento',
      };
    }
    ids.add(tipoEntradaId);
    entradas.push({ tipo_entrada_id: tipoEntradaId, aforo, precio }); // 👈
  }
